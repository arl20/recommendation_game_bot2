...
from handlers.commands import (States, add_search_game, clear_list_of_game,
                               ...
                               info_search_game, process_game_callback,
                               rating_bot_buttons, rating_buttons, set_k_games,
                               similar_search_game)
from handlers.inline_router import (add_game, delete_game, info_game,
                                    search_add_handler, search_delete_handler,
                                    search_info_handler,
                                    search_similar_handler, similar_games)
from handlers.reviews import process_review_callback, wait_review
from utils.aiogram_tests_inline import ChosenInlineHandler, InlineQueryHandler
...
async def test_cmd_start():
    start_message = """Привет! Я бот. Ты можешь написать мне свои любимые игры и получить список рекомендаций.\n
...
async def test_search_delete_handler(games_info_dict_by_name, games_info_dict):
    ...
    inline_query = InlineQuery(id="123",
                               query=f"delete {game}",
                               from_user=USER.as_object(), offset='')
...
async def test_info_game(games_info_dict_by_name, games_info_dict):
    ...
    info_game_wrapper = partial(info_game,
                                games_info_dict=games_info_dict,
                                games_info_dict_by_name=games_info_dict_by_name,
                                user_data=user_data,
                                K_GAMES=5)
...
@pytest.mark.asyncio
async def test_get_recommended_game_empty(df,
                                          games_info_dict_by_name, games_info_dict,
                                          similar_games_df,
                                          matrix,
                                          app_id_to_index,
                                          user_id_to_index):
...
async def test_get_recommendations_by_user(games_info_dict, matrix, df, user_id_to_index, app_id_to_index, k):
    ...
    recommendations_by_user = await asyncio.to_thread(get_recommendations_by_user,
                                                      matrix, user_row, 5, user_data[user_id]['k'],
